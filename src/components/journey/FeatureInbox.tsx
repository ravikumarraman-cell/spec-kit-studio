...
export function FeatureInbox({ project, onImport, onNavigate }: Props) {
  ...
  return <section className="rounded-2xl border border-violet-500/25 bg-violet-500/5 p-5">
    ...
    {items.length === 0 ? <div className="mt-4 rounded-xl border border-dashed border-zinc-700 bg-zinc-950/40 p-4 text-xs text-zinc-400"><Sparkles className="mb-2 h-4 w-4 text-violet-300" />Import a PRD, ticket, document, or pasted brief. Studio will retain the feature as a reviewable item and link its generated artifacts below.</div> : <div className="mt-4 grid gap-3 md:grid-cols-2">{items.slice().reverse().map((item) => <article key={item.id} className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate font-bold text-zinc-100">{item.title}</h3><p className="mt-1 line-clamp-2 text-xs text-zinc-400">{item.summary}</p></div><span className="shrink-0 rounded border border-violet-400/20 bg-violet-500/10 px-1.5 py-0.5 text-[10px] font-bold text-violet-200">{sourceLabels[item.source]}</span></div><div className="mt-3 flex flex-wrap gap-2 text-[10px] text-zinc-400"><span className="rounded bg-zinc-900 px-2 py-1">{item.userStoryIds.length} stories</span><span className="rounded bg-zinc-900 px-2 py-1">{item.requirementIds.length} requirements</span><span className="rounded bg-zinc-900 px-2 py-1">{item.taskIds.length} tasks</span></div><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => onNavigate('spec')} className="inline-flex items-center gap-1 text-xs font-bold text-cyan-300 hover:text-cyan-200"><FileText className="h-3.5 w-3.5" />Review spec</button><button type="button" onClick={() => onNavigate('tasks')} className="inline-flex items-center gap-1 text-xs font-bold text-emerald-300 hover:text-emerald-200"><ListTodo className="h-3.5 w-3.5" />Review tasks</button><span className="ml-auto inline-flex items-center gap-1 text-[10px] text-zinc-500">Continue <ArrowRight className="h-3 w-3" /></span></div></article>)}</div>}
  </section>;
}
