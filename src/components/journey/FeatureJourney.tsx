...
import { activeFeatureForProject, approveJourneyStage, createFeatureJourney, engineInstructionForStage, FeatureJourneyStage, featureJourneyStages, getJourneyStage, nextFeatureJourneyStage, reopenJourneyStage } from '../../lib/featureJourney';
...
interface Props {
  ...
  relatedFeatureWorkspaces?: SpecKitProject[];
  onSelectWorkspace?: (projectId: string) => void;
}

export function FeatureJourney({ project, onNavigate, onOpenFeatureImport, onSaveJourney, onSaveFeatureReview, onUpdateFeatureIdentity, relatedFeatureWorkspaces = [], onSelectWorkspace }: Props) {
  ...
  const startStage = (stage: FeatureJourneyStage) => {
    ...
    if (engineInstructionForStage(stage.id, project) && !(agentJob?.ok && agentStageId === stage.id)) { void runEngineStage(stage.id); return; }
  ...
  const runEngineStage = async (stageIdOrEvent: number | React.MouseEvent = current.id) => {
    ...
    const instruction = engineInstructionForStage(stageId, project); const agent = selectedAgent(); const repositoryPath = stageId >= 7 ? (focusedFeature?.worktreePath || project.importedRepo?.repoUrl) : project.importedRepo?.repoUrl;
    ...
    try {
      ...
      const preflight = await client.preflight(repositoryPath, project, focusedFeature?.id);
      ...
      let job = await client.startSpecKitAgent(repositoryPath, agent.id, instruction, project, focusedFeature?.id);
      setAgentJob(job);
  ...
  useEffect(() => {
    if (!discoveredArtifact || !activeFeature || (current.id !== 4 && current.id !== 5)) return;
    const acceptedAt = new Date().toISOString();
    if (current.id === 4 && !activeFeature.architecturePlan?.acceptedAt) {
  ...
  const acceptEngineReview = () => {
    ...
    if (agentStageId === 5 && officialArtifact) {
      ...
      onSaveFeatureReview({ deliveryPlan: { ...officialArtifact, acceptedAt } });
    }
  ...
  return <div className="feature-journey mx-auto max-w-5xl space-y-6 pb-12">
    ...
    {!activeFeature && relatedFeatureWorkspaces.length > 0 && <section className="rounded-2xl border border-amber-400/35 bg-amber-500/10 p-4 text-xs text-amber-100"><p className="font-bold">Feature records are retained in another Studio workspace</p><p className="mt-1 text-amber-200">The current workspace has no imported-feature receipt. Nothing is being deleted or merged automatically. Open a retained workspace below to inspect and resume its Journey.</p><div className="mt-3 flex flex-wrap gap-2">{relatedFeatureWorkspaces.map((workspace) => <button key={workspace.id} type="button" onClick={() => onSelectWorkspace?.(workspace.id)} className="rounded-lg border border-amber-300/40 bg-amber-300/10 px-3 py-2 font-bold text-amber-100 hover:bg-amber-300/20">Open {workspace.name} ({workspace.featureInbox?.length} feature{workspace.featureInbox?.length === 1 ? '' : 's'})</button>)}</div></section>}
    <FeatureRegistry project={project} />
