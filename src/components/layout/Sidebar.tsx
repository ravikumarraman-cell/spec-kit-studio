...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, project, isCollapsed = false }) => {
  ...
  if (workflowFocus === 'bug' || workflowFocus === 'assessment') {
    ...
    return <aside className="hidden shrink-0 select-none overflow-y-auto border-r border-slate-200 bg-slate-50 p-3 dark:border-zinc-800 dark:bg-zinc-950/70 md:flex md:w-72 md:flex-col">
      ...
      <div className="mt-5 px-2 text-[10px] font-black uppercase tracking-[0.16em] text-slate-500 dark:text-zinc-500">Your workflow</div><nav className="mt-2 space-y-1" aria-label={`${definition.label} steps`}>{definition.steps.map((step, index) => <button key={step.artifact} type="button" onClick={() => onTabChange('workflows')} className={`flex w-full items-center gap-2 rounded-xl px-2.5 py-2 text-left text-xs ${index === currentStep ? 'bg-cyan-500/10 text-cyan-200' : completedSteps.includes(index) ? 'text-emerald-300' : 'text-zinc-500'}`}><span className="flex h-5 w-5 items-center justify-center rounded-full border border-current text-[10px]">{completedSteps.includes(index) ? '✓' : index + 1}</span><span className="font-semibold">{step.title}</span></button>)}</nav>
  ...
  const stageLink = (item: typeof current) => {
    ...
    return <button key={item.id} type="button" onClick={() => onTabChange(item.destination)} aria-current={isCurrent ? 'step' : undefined} className={`group flex w-full items-center gap-2.5 rounded-xl border px-2.5 py-2 text-left text-xs transition-all ${isCurrent ? 'border-cyan-500/40 bg-cyan-500/10 text-slate-950 dark:text-cyan-100' : isWorkspaceOpen ? 'border-slate-300 bg-slate-200/70 text-slate-950 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-100' : 'border-transparent text-slate-700 hover:bg-slate-200/70 dark:text-zinc-400 dark:hover:bg-zinc-900'} ${!isComplete && !isCurrent ? 'opacity-70 hover:opacity-100' : ''}`}><span className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${isComplete ? 'border-emerald-500/40 bg-emerald-500/15 text-emerald-600 dark:text-emerald-300' : isCurrent ? 'border-cyan-500/50 bg-cyan-500/15 text-cyan-700 dark:text-cyan-300' : 'border-slate-300 text-slate-500 dark:border-zinc-700 dark:text-zinc-500'}`}>{isComplete ? <CheckCircle2 className="h-3.5 w-3.5" /> : item.id}</span><span className={isCurrent ? 'text-cyan-600 dark:text-cyan-300' : isComplete ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-500 dark:text-zinc-500'}>{stageIcons[item.destination]}</span><span className="min-w-0 flex-1 font-semibold">{item.shortLabel}</span>{isCurrent && <span className="rounded bg-cyan-500/15 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide text-cyan-700 dark:text-cyan-300">Now</span>}{isComplete && !isCurrent && <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-300">Edit</span>}{!isComplete && !isCurrent && <Circle className="h-2.5 w-2.5 text-slate-400 dark:text-zinc-600" />}</button>;
  };
  ...
  return <aside className="hidden shrink-0 select-none overflow-y-auto border-r border-slate-200 bg-slate-50 p-3 dark:border-zinc-800 dark:bg-zinc-950/70 md:flex md:w-72 md:flex-col">
    <button type="button" onClick={() => onTabChange('overview')} className={`rounded-xl border p-3 text-left transition-colors ${activeTab === 'overview' ? 'border-cyan-500/40 bg-cyan-500/10' : 'border-slate-200 bg-white hover:border-cyan-500/30 dark:border-zinc-800 dark:bg-zinc-900/50 dark:hover:border-cyan-500/30'}`}>
      ...
      <div className="mt-2 flex items-end justify-between gap-3"><div><p className="text-xs font-bold text-slate-900 dark:text-zinc-100">{current.id}. {current.shortLabel}</p><p className="mt-0.5 text-[11px] text-slate-500 dark:text-zinc-400">{remaining} stage{remaining === 1 ? '' : 's'} remaining</p></div><ChevronRight className="h-4 w-4 text-cyan-500" /></div>
