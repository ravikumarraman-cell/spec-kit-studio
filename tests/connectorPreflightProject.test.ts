...
test('connector preflight sends feature identity, never retained artifacts or agent output', () => {
  const project = {
    repositoryIdentity: { canonicalRemote: 'https://github.com/example/repo' },
    featureInbox: [{ id: 'feat-1', featureKey: 'US-101', slug: '101-copy-id', branch: '101-copy-id', worktreePath: '/tmp/worktree', scope: 'user-story', impactMap: { content: 'x'.repeat(2_000_000) } }],
  } as any;
