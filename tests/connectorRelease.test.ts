...
test('validates the public connector release manifest', () => {
  assert.equal(isConnectorRelease({ packageName: '@spec-kit-studio/local-connector', version: '0.1.1', apiVersion: 2, downloadPath: '/downloads/spec-kit-studio-local-connector-0.1.1.tgz' }), true);
  assert.equal(isConnectorRelease({ version: '0.1.1', apiVersion: 2, downloadPath: '/outside.tgz' }), false);
});
