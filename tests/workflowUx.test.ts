...
test('legacy Journey evidence never interrupts an imported or active workflow', () => {
  assert.equal(needsLegacyJourneyRepair({ stageId: 3, hasImpactMap: false, hasArchitecturePlan: false, hasDeliveryPlan: false }), false);
  assert.equal(needsLegacyJourneyRepair({ stageId: 4, hasImpactMap: false, hasArchitecturePlan: false, hasDeliveryPlan: false }), false);
  assert.equal(needsLegacyJourneyRepair({ stageId: 5, hasImpactMap: true, hasArchitecturePlan: false, hasDeliveryPlan: false }), false);
  assert.equal(needsLegacyJourneyRepair({ stageId: 6, hasImpactMap: false, hasArchitecturePlan: false, hasDeliveryPlan: false }), false);
